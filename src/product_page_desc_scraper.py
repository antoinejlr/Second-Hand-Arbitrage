...
def fetch_product_desc_from_str(html: str) -> str:
    ...
    description = bs.div.div.section.contents[-4].div.div.div.contents[3].div.div.div.section.contents[2] \
        .div.div.contents[2].article.p.text
    return description
...
def fetch_product_desc_from_file(file_name: str) -> tuple:
    ...
    with open(file, 'r') as f:
        ...
        body_description = \
            bs.div.div.section.contents[1].div.div.div#.contents[3].div.div.div.section.contents[
               # 2].div.div.contents[-1].article.get_text(separator="\n", strip=True)
        header_description = \
            bs.div.div.section.contents[1].div.div.div.contents[3].div.div.div.section.contents[
                2].div.div.contents[-2].text
...
def append_to_url_filename_description_mapping(url, file_name, header_description, body_description):
    with open('/Users/Shared/github_projects/Second-Hand-Arbitrage/metadata/url_filename_description_mapping.csv',
              "a") as file:
...
def append_url_to_list_of_urls(url):
    with open('/Users/Shared/github_projects/Second-Hand-Arbitrage/metadata/product_page_urls.csv',
              "a") as file:
...
def load_product_pages(product_page_urls: list) -> None:
    ...
    for product_page_url in tqdm(product_page_urls):
        ...
        # Step 3
        header_description, body_description = fetch_product_desc_from_file(product_file_name)
        append_to_url_filename_description_mapping(product_page_url, product_file_name,
                                                   header_description, body_description)
...
def fetch_new_product_urls() -> list:
    ...
    new_product_page_urls = list(set(pd.merge(latest_file_df,
